#' @title  RNA-seq subsampling
#' @description A simplified method to subsample a given RNA-seq data to a
...
#'
#' @author Alemu Takele Assefa (alemutak@hotmail.com)
#'
#' @param counts orginal (unormalized) RNA-seq count matrix (genes in row and samples in column)
#' @param prop A binomial probability (0<prop<1). Either a single value to subsample the
...
#' count matrix.
#' @param target.LS The desired library size per sample (if prop is not specified).
#' It can be a single value or a vector of dimension equal to the number of  samples.
#' @param target.total.count The desired total number of reads (if prop or target.LS
...
#'
#' @references David Robinson, John D. Storey and with contributions from Andrew J.
...
#'
#'
#' @details Let X is a Gxn matrix of un-normalized read counts with genes in rows and samples
